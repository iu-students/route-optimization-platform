...
class Point:
    ...
    def __init__(
        ...
        vehicle: Vehicle, end_time: float, vehicle_time: float,
        ...
        for j in i.assigned_loaders:
            if not j.has_mandatory_point:
                i.point_cost += (
                    ...
                    + loader_salary / max(len(j.route) - 1, 1)
                )
                path_cost = 0.0
                if len(j.route) > 2:
                    idx = j.route.index(i)
...
def remove_disadvantageous_and_rerun(
    ...
    for route in cleaned["routes"]:
        new_points = [
            p for p in route["points"] if p["id"] not in bad_point_ids
        ]
    ...
    print(
        f"[rerun] Удалено невыгодных точек: {removed_count}, "
...
def run_with_data(data: dict):
    ...
    for loader in loaders:
        loader.loader_full_salary = (
            loader_salary
            + (loader_shift_size - loader_shift_time_left_snapshot(loader))
            * loader_work
        )
...
def solve_loaders():
    global loader_shift_size, loader_speed, loader_salary, loader_work
    global optional_point_penalty
