...
- **Pipeline A** (`CP-SAT/main.py`, `CP-SAT/vehicle_routes.py`, `CP-SAT/loader_routes.py`) — generates a
  route pool via Clarke-Wright and insertion heuristics, selects the optimal
  subset with OR-Tools CP-SAT, then runs two post-processing passes
  (`consolidate_routes`, `merge_multi_trip_routes`) to further reduce the
  vehicle count before assigning loaders.
- **Pipeline B** (`PyVRP/script.py`, `PyVRP/loaders.py`) — uses the PyVRP library for vehicle
  routing and a greedy chain-building heuristic for loader assignment.

Only Pipeline A is wired to the Flask API (`Web/app.py`). Pipeline B is invoked
